...
func change_scene(new_scene: String, loading_icon: String = "res://addons/arc_loader/loading_icon.tscn"):
	...
	if current_scene.scene_file_path == new_scene:
		log_err("Cannot load the currently active scene, use get_tree().reload_current_scene() instead")
		return
